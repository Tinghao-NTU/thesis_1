...
for ep_iter in range(1):
    ...
    for roll_num in range(2,6):
        ...
        print('roll out')        
        while not done: 
            beita = np.random.randint(0, 2, size=[100])
            for i in range(2):
                ...
                beita = np.clip( np.log( 1000 * g * g/(N0 * D * np.log(2))) * gamma * B * t_k/D, 0, 1)
            ...
            print(F)
            print(f)
            print(F[selected_devices_idx])
            print(f_list[selected_devices_idx])
            ...
            next_state_matrix,net_glob,acc_list,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,
                                       dict_users,idxs_users,epoch,net_glob,acc_list = acc_list, w_list = w_list, name =name1+'radio_cifar_'+str(roll_num)+'.txt',preset = preset_accuracy)            
        ...
        file_handle=open('Devices_num.txt',mode='a')
        file_handle.write(str(np.mean(selected_nums)))
        file_handle.write('\n')
        file_handle.close()
        
        file_handle=open('T_radio.txt',mode='a')
        file_handle.write(str(T))
        file_handle.write('\n')
        file_handle.close()
        file_handle=open('E_radio.txt',mode='a')
